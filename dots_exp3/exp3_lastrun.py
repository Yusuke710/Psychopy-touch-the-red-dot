...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Yusuke\\Desktop\\dots_exp3\\exp3_lastrun.py',
...
stimuli_prac = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
practice_dot = visual.DotStim(
    win=win, name='practice_dot',
    nDots=1, dotSize=20.0,
...
dots = visual.DotStim(
    ...
    nDots=1, dotSize=20.0,
    speed=0.0, dir=0.0, coherence=0.0,
    fieldPos=[0,0], fieldSize=1.0,fieldShape='circle',
    signalDots='same', noiseDots='direction',dotLife=0.0,
    color='red', colorSpace='rgb', opacity=None,
    depth=-1.0)
...
# update component parameters for each repeat
# keep track of which components have finished
practice_stimuliComponents = [stimuli_prac, practice_dot]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # *practice_dot* updates
    if practice_dot.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        practice_dot.frameNStart = frameN  # exact frame index
        practice_dot.tStart = t  # local t and not account for scr refresh
        practice_dot.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(practice_dot, 'tStartRefresh')  # time at next scr refresh
        practice_dot.setAutoDraw(True)
    if practice_dot.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > practice_dot.tStartRefresh + 0.2-frameTolerance:
            # keep track of stop time/frame for later
            practice_dot.tStop = t  # not accounting for scr refresh
            practice_dot.frameNStop = frameN  # exact frame index
            win.timeOnFlip(practice_dot, 'tStopRefresh')  # time at next scr refresh
            practice_dot.setAutoDraw(False)
...
if len(click.x): thisExp.addData('click.x', click.x[0])
if len(click.y): thisExp.addData('click.y', click.y[0])
if len(click.leftButton): thisExp.addData('click.leftButton', click.leftButton[0])
if len(click.midButton): thisExp.addData('click.midButton', click.midButton[0])
if len(click.rightButton): thisExp.addData('click.rightButton', click.rightButton[0])
if len(click.time): thisExp.addData('click.time', click.time[0])
...
for thisTrial_0 in trials_0:
    ...
    dots.setFieldPos(location)
    # keep track of which components have finished
    stimuliComponents = [stimuli1, dots]
    ...
    for thisComponent in mouseComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store data for trials_0 (TrialHandler)
    if len(click.x): trials_0.addData('click.x', click.x[0])
    if len(click.y): trials_0.addData('click.y', click.y[0])
    if len(click.leftButton): trials_0.addData('click.leftButton', click.leftButton[0])
    if len(click.midButton): trials_0.addData('click.midButton', click.midButton[0])
    if len(click.rightButton): trials_0.addData('click.rightButton', click.rightButton[0])
    if len(click.time): trials_0.addData('click.time', click.time[0])
    trials_0.addData('click.started', click.tStart)
    trials_0.addData('click.stopped', click.tStop)
    # the Routine "mouse" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisTrial_1 in trials_1:
    ...
    if len(click.x): trials_1.addData('click.x', click.x[0])
    if len(click.y): trials_1.addData('click.y', click.y[0])
    if len(click.leftButton): trials_1.addData('click.leftButton', click.leftButton[0])
    if len(click.midButton): trials_1.addData('click.midButton', click.midButton[0])
    if len(click.rightButton): trials_1.addData('click.rightButton', click.rightButton[0])
    if len(click.time): trials_1.addData('click.time', click.time[0])
    trials_1.addData('click.started', click.tStart)
    trials_1.addData('click.stopped', click.tStop)
    # the Routine "mouse" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisTrial_2 in trials_2:
    ...
    if len(click.x): trials_2.addData('click.x', click.x[0])
    if len(click.y): trials_2.addData('click.y', click.y[0])
    if len(click.leftButton): trials_2.addData('click.leftButton', click.leftButton[0])
    if len(click.midButton): trials_2.addData('click.midButton', click.midButton[0])
    if len(click.rightButton): trials_2.addData('click.rightButton', click.rightButton[0])
    if len(click.time): trials_2.addData('click.time', click.time[0])
    trials_2.addData('click.started', click.tStart)
    trials_2.addData('click.stopped', click.tStop)
    # the Routine "mouse" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisTrial_3 in trials_3:
    ...
    if len(click.x): trials_3.addData('click.x', click.x[0])
    if len(click.y): trials_3.addData('click.y', click.y[0])
    if len(click.leftButton): trials_3.addData('click.leftButton', click.leftButton[0])
    if len(click.midButton): trials_3.addData('click.midButton', click.midButton[0])
    if len(click.rightButton): trials_3.addData('click.rightButton', click.rightButton[0])
    if len(click.time): trials_3.addData('click.time', click.time[0])
    trials_3.addData('click.started', click.tStart)
    trials_3.addData('click.stopped', click.tStop)
    # the Routine "mouse" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
